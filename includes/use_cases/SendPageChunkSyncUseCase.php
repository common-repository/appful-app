...
class SendPageChunkSyncUseCase {
	...
	public function invoke( SyncChunk $sync_items ) {
		...
		Logger::debug( "Sending page chunk to sync: " . json_encode( $sync_item_dtos ) );
		$request = HttpRequest::backend_request()->method( "PATCH" )->body( $sync_item_dtos )->path( Constants::$PAGE_PATH . "/sync" );
