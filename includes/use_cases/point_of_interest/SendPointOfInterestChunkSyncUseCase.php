...
class SendPointOfInterestChunkSyncUseCase {
	...
	public function invoke( SyncChunk $sync_items ) {
		...
		Logger::debug( "Sending point of interest chunk to sync: " . json_encode( $sync_item_dtos ) );

		$request = HttpRequest::backend_request()->method( "PATCH" )->body( $sync_item_dtos )->path( Constants::$POINT_OF_INTEREST_PATH . "/sync" );
